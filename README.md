...
- `/ban member duration? reason?` - ban a member; use `30m` for minutes, `2h` for hours, `3d` for days, or leave duration blank for permanent.
- `/unban user_id reason?` - unban a user by ID.
- `/timeout member duration reason?` - timeout a member for up to 28 days; use `30m` for thirty minutes, `1h` for one hour, or `2d` for two days.
- `/removetimeout member` - remove a member's timeout.
- `/servermute member duration?` - create or use a `Muted` role; use `30m` for thirty minutes, `1h` for one hour, `2d` for two days, or leave duration blank for permanent.
