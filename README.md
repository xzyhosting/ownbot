...
- `/timeout member minutes reason?` - timeout a member for 1 minute to 28 days.
- `/removetimeout member` - remove a member's timeout.
- `/servermute member` - create or use a `Muted` role and deny messaging and voice permissions.
- `/unmute member` - remove the `Muted` role from a member.
- `/afk message? minutes?` - set your AFK status and optional duration (1 minute to 7 days); posting clears it. The bot stays silent when an AFK user is mentioned.
