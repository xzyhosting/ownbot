...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('duration').setDescription('Ban duration, e.g. 30m, 2h or 3d. Blank is permanent.'))
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('duration').setDescription('Timeout duration, e.g. 30m, 1h or 2d.').setRequired(true))
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('duration').setDescription('Mute duration, e.g. 30m, 1h or 2d. Blank is permanent.'))
...
function parseDuration(value) {
  ...
  const milliseconds = amount * (match[2] === 'm' ? 60 * 1000 : match[2] === 'h' ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000);
  ...
  try {
    ...
    if (interaction.commandName === 'kick' || interaction.commandName === 'ban' || interaction.commandName === 'timeout' || interaction.commandName === 'servermute') {
      ...
      if (interaction.commandName === 'kick') {
        ...
        if (duration === 0) {
          await interaction.reply({ content: 'Invalid duration. Use a format like 30m, 2h or 3d, or leave it blank for a permanent ban.', ephemeral: true });
          return;
        }
        ...
        if (!duration || duration > 28 * 24 * 60 * 60 * 1000) {
          await interaction.reply({ content: 'Invalid timeout duration. Use 1m to 28d, for example 30m, 1h or 2d.', ephemeral: true });
          return;
        }
        ...
        const duration = parseDuration(durationInput);
        if (duration === 0) {
          await interaction.reply({ content: 'Invalid mute duration. Use a format like 30m, 1h or 2d, or leave it blank for a permanent mute.', ephemeral: true });
          return;
        }
